...
const DATA_SQL = `
  ...
  ORDER BY
    CASE product_code_grouping
      WHEN 'SHORT_TERM_POWER' THEN 1
      WHEN 'POWER_OPTIONS' THEN 2
      WHEN 'POWER_FUTURES' THEN 3
      WHEN 'BASIS' THEN 4
      WHEN 'BALMO' THEN 5
      WHEN 'GAS_FUTURES' THEN 6
      WHEN 'GAS_OPTIONS' THEN 7
      ELSE 999
    END
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("[marex-allocated-trades] DB query failed:", message);
    return NextResponse.json(
      { error: "Failed to fetch Marex allocated trades data", detail: message },
      { status: 500 }
    );
  }
}
