...
type Filters = Partial<Record<keyof TradeRow, string>>;

interface TradeRow {
  sftp_date: string;
  trade_date: string;
  clear_street_account: string;
  exchange_code: string;
  product_code_grouping: string;
  product_code_region: string;
  is_option: boolean;
...
const COLUMNS: Column[] = [
  ...
  { key: "clear_street_account", label: "Account", align: "left", format: fmtStr, filterable: true },
  { key: "exchange_code", label: "Exchange", align: "left", format: fmtStr, filterable: true },
  { key: "product_code_grouping", label: "Product Group", align: "left", format: fmtStr, filterable: true },
  { key: "product_code_region", label: "Region", align: "left", format: fmtStr, filterable: true },
  ...
  { key: "contract_day", label: "Day", align: "right", format: fmtInt },
  { key: "contract_description", label: "Description", align: "left", format: fmtStr },
  { key: "trade_price_total", label: "Trade Price", align: "right", format: (v) => fmtNum(v, 3) },
  { key: "qty_total", label: "Qty Total", align: "right", format: fmtInt, filterable: true },
  { key: "qty_acim", label: "Qty ACIM", align: "right", format: fmtInt, filterable: true },
  { key: "qty_pnt", label: "Qty PNT", align: "right", format: fmtInt, filterable: true },
  { key: "qty_dickson", label: "Qty Dickson", align: "right", format: fmtInt, filterable: true },
  { key: "qty_titan", label: "Qty Titan", align: "right", format: fmtInt, filterable: true },
];
...
export default function ClearStreetIntradayTrades() {
  ...
  const setFilter = useCallback((key: keyof TradeRow, value: string) => {
    setFilters((prev) => {
      if (!value) {
        const next = { ...prev };
        delete next[key];
  ...
  useEffect(() => {
    ...
    const url = selectedDate
      ? `/api/clear-street-intraday-trades?sftp_date=${selectedDate}`
      : "/api/clear-street-intraday-trades";
    ...
    fetch(url, { signal: controller.signal })
      ...
      .catch((err) => {
        ...
        setError(err.message || "Failed to load Clear Street intraday trades data");
      })
  ...
  // Unique values for each filterable column (derived from full data set)
  const filterOptions = useMemo(() => {
    const opts: Partial<Record<keyof TradeRow, string[]>> = {};
  ...
  const filteredData = useMemo(() => {
    const activeFilters = Object.entries(filters) as [keyof TradeRow, string][];
  ...
  const groupCounts = useMemo(() => {
    ...
    for (const row of filteredData) {
      const g = row.product_code_grouping || "OTHER";
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">
            {filteredData.length}{activeFilterCount > 0 ? ` of ${data.length}` : ""} trades
          </p>
        </div>
        {activeFilterCount > 0 && (
          <button
        ...
        <div className="rounded-xl border border-gray-800 bg-[#0f1117] px-4 py-3">
          ...
          <div className="flex items-end gap-3">
            <div className="flex flex-col gap-1">
              <label htmlFor="sftp-date-filter-intraday" className="text-[10px] font-medium text-gray-500 uppercase tracking-wider">
                SFTP Date
              </label>
              <select
                id="sftp-date-filter-intraday"
                value={pendingDate}
              ...
              {filteredData.map((row, idx) => (
                <tr
                  ...
                  className={`border-b border-gray-800/50 hover:bg-gray-800/30 ${
                    GROUPING_COLORS[row.product_code_grouping] ||
      ...
      {!loading && !error && filteredData.length === 0 && (
        <div className="flex items-center justify-center h-48">
          <div className="text-gray-500">
            {data.length > 0 ? "No trades match the selected filters" : "No trades found"}
          </div>
        </div>
      )}
    </div>
