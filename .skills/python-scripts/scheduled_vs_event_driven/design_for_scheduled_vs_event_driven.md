...
1. [Teammate 1: Scheduled vs Event-Driven Python Script Analysis](#teammate-1-scheduled-vs-event-driven-python-script-analysis)
2. [Teammate 2: PostgreSQL Trigger Analysis](#teammate-2-postgresql-trigger-analysis)
3. [Teammate 3: Event-Driven Implementation Plan](#teammate-3-event-driven-implementation-plan)
...
### 8. When to Use Which: Decision Criteria

#### Use SCHEDULED when:

| Criterion | Explanation |
|-----------|-------------|
| Known, reliable publication time | A cron at the known time reliably catches data |
| Historical backfill is primary use case | Pulling last N days is inherently time-based |
| No upstream database event exists | Nothing to trigger on yet |
| Simplicity is paramount | "This runs at 2 PM" is easy to reason about |
| Rate-limited APIs with fixed windows | Must call at specific times |

#### Use EVENT-DRIVEN when:

| Criterion | Explanation |
|-----------|-------------|
| Downstream depends on upstream data landing | A dependency relationship, not a schedule |
| Upstream arrival time is unpredictable | Fixed schedule either runs too early or too late |
| Chain of dependent transformations | A -> B -> C pipelines are naturally event-driven |
| Near-real-time requirements | Eliminates polling delay |
| Multiple consumers of the same event | Decouples cleanly |

#### Hybrid Pattern (recommended):

Use **scheduled** for the initial scrape (external API -> raw table) and **event-driven** for all downstream processing (raw table -> derived tables, alerts, exports). This gives:
- Predictable, retryable external API calls (scheduled)
- Reactive, low-latency internal processing (event-driven)
- The scheduled script's lookback window acts as automatic catch-up for missed events
