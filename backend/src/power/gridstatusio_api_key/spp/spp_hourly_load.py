...
def _format(
    ...
    # dtypes
    load_columns = ['csws', 'ede', 'grda', 'indn', 'kacy', 'kcpl', 'les', 'mps', 'nppd', 'okge', 'oppd', 'seci', 'sprm', 'sps', 'waue', 'wfec', 'wr', 'system_total']
    for column in load_columns:
        if column not in df.columns:
            df[column] = 0.0
    df[load_columns] = df[load_columns].astype(float)
