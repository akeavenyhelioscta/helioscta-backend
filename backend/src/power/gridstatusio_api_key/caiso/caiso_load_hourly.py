...
def _pull(
    ...
    df = client.get_dataset(
        dataset="caiso_load_hourly",
        start=start_date.strftime("%Y-%m-%d"),
        end=end_date.strftime("%Y-%m-%d"),
        filter_column="tac_area_name",
        filter_operator="in",
        filter_value=[
            "CA ISO-TAC",
            "PGE",
...
def _upsert(
    ...
    primary_key_candidates = ['interval_start_local', 'interval_start_utc', 'interval_end_local', 'interval_end_utc', 'tac_area_name']
...
def main(
        dates: list = [(datetime.now() - timedelta(days=30), datetime.now() + timedelta(days=1))],
    ):
