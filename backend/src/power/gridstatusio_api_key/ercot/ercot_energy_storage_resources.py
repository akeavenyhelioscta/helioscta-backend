...
def _get_timestamp_columns(
    ...
    # drop time
    df = df.drop(columns=[time_column, utc_time_column, interval_start_column, interval_end_column])
...
def main(
        dates: list = [(datetime.now() - timedelta(days=14), datetime.now())],
    ):
