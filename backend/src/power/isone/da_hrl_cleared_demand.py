...
def _pull(
        start_date: datetime.date = (datetime.now() + timedelta(days=1)).date(),
        end_date: datetime.date = (datetime.now() + timedelta(days=1)).date(),
    ) -> pd.DataFrame:
    """
    ...
    # build url
    url: str = f"https://www.iso-ne.com/transform/csv/hourlydayaheaddemand?start={start_date.strftime('%Y%m%d')}&end={end_date.strftime('%Y%m%d')}"
